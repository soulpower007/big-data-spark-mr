...
join = spark.sql("""
                  select trips2._c18 as medallion_type, count(*) as total_trips, 
                  round(sum(trips2._c5),2) as  total_revenue, 
                  round(sum(trips2._c8),2)*100/ round(sum(trips2._c5),2)  as avg_tip_percentage
                  from trips2
                  group by trips2._c18
                  order by trips2._c18""")
...
join.show()
join.select(format_string('%s,%s,%.2f,%.2f',join.medallion_type,  join.total_trips, join.total_revenue, join.avg_tip_percentage)).write.save('task4b-sql.out', format='text')
